...
public class A_StringPool {
	...
	// - 문자열 데이터가 담기는 영역으로, Heap메모리 영역상에 항상 고정적으로 할당되어 있다.
	public void method1() {
		String str1 = new String("hello");
		String str2 = new String("hello");
		
		System.out.println(str1 == str2); // false(주소값 비교)
		System.out.println(str1); // toStirng()메서드가 오버라이딩 되어 있음.
		...
		// String의 hashCode()메서드의 경우 주소값이 아닌 담긴 무자열을 10진수로
		// 변환해주는 메서드로 오버라이딩 되어 있다.
		
		// 실제 참조변수의 주소값을 확인하는 메서드
		// System.identifyHashCode(변수);
	...
	public void method2() {
		 ...
		 * 
		 * 문자열 상수풀 동작방식(jvm)
		 * 1. 프로그램 실행 도중 문자열 리터럴이 만나면, 우선 문자열 리터럴값이 이미
		 * 	  상수풀에 보관되어 있나 검사를 진행(intern)
		 * 2. 만약 상수풀에 저장된 리터럴 값이 아니라면 문자열의 hashCode함수를 호출하고
		 * 	  그 값을 상수들의 주소값으로 활용하여 값을 저장한다.
		 * 3. 만약 이미 상수풀에 그 값이 저장되어 있다면, 저장되어 있는 주소값을 반환한다. (얕은복사)
		 */
	 ...
	 * 
	 * 단, 문자열상수풀로도 감당이 불가능할 정도로 문자열 연산이 자주 사용되는 프로그램이라면
