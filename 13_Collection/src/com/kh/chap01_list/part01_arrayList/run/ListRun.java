...
public class ListRun {
	 ...
	 *  - 효율적인 기능들이 정의되어있는 틀
	 *  
	 * 데이터들이 새롭게 추가되거나 삭제가되거나, 수정이 되는 기능들을 이미 정의해놓은 틀이다.(컬렉션)
	 * 
	 * 배열과 컬렉션(List)의 차이점
	 *  - 배열의 단점
	 *  1. 같은자료형의 데이터만 저장가능
	 ...
	 *  2. 중간에 값을 추가하거나, 삭제하는 등 배열내부의 값들을 조작하기위한 편리한 메서드가 정의
	 *     되어 있다.
	 *  3. 크기에 대한 제약이 없다. 
	 *  
	 *  
	 ...
	 * - 중복값을 담아도 상관없다.
	 * - ArrayList, Vector, LinkedList들이 존재
	 * 
	 * Set계열
	 * - 담고자하는 값만 저장
	 * - 값을 저장시 순서를 유지하지 않는다(index가 없다)
	 * - 중복값을 허용하지 않는다.
	 * - HashSet, TreeSet
	 * 
	 * Map계열
	...
	public static void main(String[] args) {
		 ...
		 *  - 제네릭을 설정하면 설정한 타입의 값들을 컬렉션에 담을 수 있다. 
		 *  
		 * 제네릭을 설정하는 이유
		...
		ArrayList<Music> list = new ArrayList<Music>(3); // 객체 생성시에는 제네릭 생략가능.
		...
		// 중간에 값 추가시 알아서 기존값들을 뒤로 한칸씩 땡겨주는 깊은복사가 내부적으로 발생한다.
